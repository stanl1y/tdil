...
class Maze_v5(gym.Env):
    """Custom Environment that follows gym interface"""

    metadata = {"render.modes": ["human"]}

    def __init__(self, maze_size=10, random_reset=False):
        super(Maze_v5, self).__init__()
        ...
        if not os.path.exists(
            "./saved_expert_transition/Maze-v5/oracle/episode_num1.pkl"
        ):
            with open(
                "./saved_expert_transition/Maze-v5/oracle/episode_num1.pkl", "wb"
            ) as f:
                data = {}
        ...
        plt.imsave(f"environment/custom_env/Maze-v5.png", tmp)
    ...
    def step(self, action):
        ...
        if (
            ...
            or y < 0
            or y >= self.maze_size
            or (x >= 3 and (3 in hor_set) and (4 in hor_set))
            or (x >= 4 and x < 9 and (6 in hor_set) and (7 in hor_set))
            or (y >= 7 and (8 in ver_set) and (9 in ver_set))
        ):
